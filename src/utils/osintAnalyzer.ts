...
class OSINTAnalyzer {
  ...
  async analyzeInput(input: OSINTInput): Promise<OSINTAnalysisResult> {
    ...
    switch (input.type) {
      case 'url':
        ...
        break;
      case 'domain':
        await this.analyzeDomainWithWhois(input, result);
        break;
      case 'subdomain':
      ...
      case 'username':
        ...
        break;
      case 'social_media':
        await this.analyzeSocialMediaWithSherlock(input, result);
        break;
      case 'phone':
        await this.analyzePhoneWithInspector(input, result);
        break;
      case 'mac':
    ...
    switch (type) {
      ...
      case 'social_media':
        ...
        return usernameRegex.test(value)
          ? { isValid: true }
          : { isValid: false, error: 'Invalid username format (alphanumeric, dots, underscores, hyphens, max 30 chars)' };
  ...
  private getFormatDescription(type: OSINTInputType): string {
    const descriptions = {
      ...
      social_media: 'Username for social media enumeration'
    };
  ...
  // Enhanced domain analysis with WHOIS integration
  private async analyzeDomainWithWhois(input: OSINTInput, result: OSINTAnalysisResult): Promise<void> {
    try {
      // Call our backend API for WHOIS data
      ...
      if (response.ok) {
        ...
        // Populate network info with WHOIS data
        result.networkInfo = {
          ip: this.generateRandomIP(),
          location: {
            country: 'Unknown',
            ...
            coordinates: { lat: 0, lng: 0 },
            isp: 'Unknown',
            organization: whoisData.whois_data?.registrant_org || 'Unknown'
          },
          dns: {
            records: [],
            nameservers: whoisData.whois_data?.name_servers || [],
            mx: []
          }
        };

        // Add WHOIS-specific intelligence
        result.webIntelligence = {
          ...result.webIntelligence,
          whoisData: {
            registrar: whoisData.whois_data?.registrar || 'Unknown',
            creationDate: whoisData.whois_data?.creation_date || 'Unknown',
            expirationDate: whoisData.whois_data?.expiration_date || 'Unknown',
            updatedDate: whoisData.whois_data?.updated_date || 'Unknown',
            nameServers: whoisData.whois_data?.name_servers || [],
            registrantOrg: whoisData.whois_data?.registrant_org || 'Unknown',
            adminEmail: whoisData.whois_data?.admin_email || 'Unknown',
            status: whoisData.whois_data?.status || []
          }
        };

        result.aiSummary.keyFindings.push(`Domain registered with ${whoisData.whois_data?.registrar}`);
        result.aiSummary.keyFindings.push(`Created: ${whoisData.whois_data?.creation_date}`);
        result.aiSummary.keyFindings.push(`Expires: ${whoisData.whois_data?.expiration_date}`);
        
      } else {
        // Fallback to existing web analysis
        await this.analyzeWebTarget(input, result);
      }
    } catch (error) {
      console.error('WHOIS analysis failed:', error);
      // Fallback to existing web analysis
      await this.analyzeWebTarget(input, result);
    }
  }

  // Social media enumeration with Sherlock integration
  private async analyzeSocialMediaWithSherlock(input: OSINTInput, result: OSINTAnalysisResult): Promise<void> {
    try {
      // Call our backend API for Sherlock analysis
      const response = await fetch('http://localhost:3001/api/sherlock', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        const sherlockData = await response.json();
        
        const platforms = sherlockData.results.map((site: any) => ({
          platform: site.site || site.name || 'Unknown',
          username: input.value,
          url: site.url || `https://${site.site?.toLowerCase()}.com/${input.value}`,
          verified: site.status === 'found',
          found: site.status === 'found',
          responseTime: site.response_time || null,
          profileData: {
            name: input.value,
            status: site.status
          }
        ...
        result.socialIntelligence = {
          ...
          connections: [],
          reputation: {
            score: Math.floor((sherlockData.found_profiles / sherlockData.total_sites) * 100),
            sources: ['Sherlock'],
            positiveReviews: sherlockData.found_profiles,
            negativeReviews: 0
          },
          sherlockAnalysis: {
            totalSites: sherlockData.total_sites,
            foundProfiles: sherlockData.found_profiles,
            timestamp: sherlockData.timestamp
          }
        };

        result.aiSummary.keyFindings.push(`Found ${sherlockData.found_profiles} profiles out of ${sherlockData.total_sites} sites checked`);
        result.aiSummary.keyFindings.push(`Digital footprint score: ${Math.floor((sherlockData.found_profiles / sherlockData.total_sites) * 100)}%`);
        
        if (sherlockData.found_profiles > 5) {
          result.aiSummary.riskLevel = 'medium';
          result.aiSummary.keyFindings.push('High social media presence detected');
        }
        
      } else {
        // Fallback to existing username analysis
        await this.analyzeUsername(input, result);
      }
    } catch (error) {
  ...
  // Enhanced phone analysis with Inspector integration
  private async analyzePhoneWithInspector(input: OSINTInput, result: OSINTAnalysisResult): Promise<void> {
    try {
      // Call our backend API for Inspector analysis
      ...
      if (response.ok) {
        ...
        result.phoneIntelligence = {
          carrier: inspectorData.results?.carrier_info?.carrier || 'Unknown',
          type: inspectorData.results?.phone_info?.type || 'mobile',
          location: {
            country: inspectorData.results?.phone_info?.country || 'Unknown',
            region: inspectorData.results?.phone_info?.region || 'Unknown',
            timezone: inspectorData.results?.phone_info?.timezone || 'Unknown'
          },
          spam: {
            score: 0,
            reports: 0,
          ...
          linkedAccounts: Object.entries(inspectorData.results?.social_media || {}).map(([platform, status]) => ({
            service: platform,
            verified: status === 'found'
          })),
          inspectorAnalysis: {
            timestamp: inspectorData.timestamp,
            socialMediaFound: Object.values(inspectorData.results?.social_media || {}).filter(status => status === 'found').length,
            reputationData: inspectorData.results?.reputation || {}
          }
        };

        const socialMediaCount = Object.values(inspectorData.results?.social_media || {}).filter(status => status === 'found').length;
        
        result.aiSummary.keyFindings.push(`Phone number analysis completed using Inspector`);
        if (socialMediaCount > 0) {
          result.aiSummary.keyFindings.push(`Found ${socialMediaCount} linked social media accounts`);
        }
        
        if (inspectorData.results?.reputation) {
          result.aiSummary.keyFindings.push('Reputation data available');
        }
        
      } else {
        // Fallback to existing phone analysis
